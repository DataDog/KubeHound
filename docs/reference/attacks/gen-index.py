...
for file in sorted(glob.glob('*.md')):
  ...
  with open(file, 'r') as f:
    contents = f.read()
    if contents.startswith(COMMENT_PREFIX):
      print("Parsing", file)
      docs_config = yaml.safe_load(contents[len(COMMENT_PREFIX):contents.find(COMMENT_SUFFIX)])
      ...
      table += f'| [{docs_config["id"]}](/attacks/{file[:-len(".md")]}) | {docs_config["name"]} | {attackTechniqueName} | { attackTacticName} | \n'
    else:
