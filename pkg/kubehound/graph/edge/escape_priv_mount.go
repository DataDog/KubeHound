...
func (e *EscapePrivMount) Stream(ctx context.Context, db *sql.DB, w types.EdgeWriter) error {
	oic := converter.NewObjectID(db)
	rows, err := db.QueryContext(ctx, `SELECT id, node_id FROM containers WHERE privileged = 1 AND run_id = ? AND cluster_name = ?`,
