...
var assessorInstance *RiskAssessor
var raOnce sync.Once

// Registered returns the edge registry singleton.
func Assessor() *RiskAssessor {
	var err error
	raOnce.Do(func() {
		assessorInstance, err = NewAssessor()
		if err != nil {
			log.I.Fatalf("Risk assessor initialization: %v", err)
		}
	})
...
func NewAssessor() (*RiskAssessor, error) {
...
func (ra *RiskAssessor) IsCritical(model any) bool {
	switch o := model.(type) {
	case *store.Role:
