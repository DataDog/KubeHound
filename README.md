...
+ Use the default password `admin` to login (note: this can be changed via the [Dockerfile](./deployments/kubehound/notebook/Dockerfile) or by setting the `JUPYTER_PASSWORD` environment variable in the [.env](./deployments/kubehound/.env.tpl) file)
